...
开启 `enableQuote` 后，`cs2log.test` 发送的每一条测试新闻都会引用触发该命令的消息；关闭后则直接发送新闻内容。
...
`cs2log.push`、`cs2log.test` 与 `cs2log.ai` 会在当前会话报告字体、图片或 LLM 翻译 fallback；自动轮询只写插件日志，不会在正式公告后附加技术提示。LXGW WenKai 遵循 SIL Open Font License 1.1。
...
- `llmApiEndpoint`：API 基础地址或完整端点，默认 `https://api.deepseek.com`。Anthropic 自动补 `/v1/messages`，OpenAI 自动补 `/v1/chat/completions`；已有完整路径时保持原样。
- `llmModel`：共用模型名，默认 `deepseek-v4-flash`。
- `llmMaxTokens`：最大输出 Token 数，默认 10240，可设置 32～102400。
- `llmTranslateTimeout`：普通新闻翻译单次请求超时，单位为秒，默认 600，可设置 30～3600。
- `llmSummaryTimeout`：`cs2log.ai` 摘要单次请求超时，单位为秒，默认 600，可设置 30～3600。
- `llmTranslatePrompt`：逐篇新闻翻译提示词。
- `llmSummaryPrompt`：合并摘要提示词，决定摘要的内容、格式与输出语言。

使用 Anthropic 格式时，只需将 `llmApiFormat` 改为 `anthropic`。例如填写 `http://localhost:3000` 时，实际请求地址为 `http://localhost:3000/v1/messages`；OpenAI 格式则会请求 `http://localhost:3000/v1/chat/completions`。

OpenAI 格式默认发送兼容服务常见的 `max_tokens`；如果接口明确拒绝该参数，插件会自动改用 `max_completion_tokens` 重试一次。

翻译出现配置错误、请求超时、输出截断、响应格式错误或普通请求失败时，插件会保留原文继续推送。手动 `cs2log.test` 与 `cs2log.push` 会报告具体回退原因，自动轮询仅记录日志。

使用支持 DeepSeek V4 模型后缀的 New API 时，可手动填写 `deepseek-v4-pro-none` 或 `deepseek-v4-flash-none` 关闭思考以降低翻译耗时。该后缀不是通用标准，插件不会自动修改用户填写的模型名。
